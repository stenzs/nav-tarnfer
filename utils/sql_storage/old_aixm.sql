...
ContactInformationTelephoneContact, ContactInformationPostalAddress, AirportHeliportCity, Service,
...
AirspaceActivation_OrganisationAuthority, SignificantPointInAirspace, SignificantPoint, Curve,
AirportHeliport_AirportGroundService, Unit, UnitDependency, CallsignDetail, radiocommunicationchannel,
service_radiocommunicationchannel, trafficseparationservice, airspace_airtrafficmanagementservice, airtrafficcontrolservice,
AuthorityForAirspace, Navaid, GroundLightingAvailability, groundtrafficcontrolservice, AircraftGroundService,
OrganisationAuthority_PropertiesWithSchedule, PropertiesWithSchedule, Timesheet, airportheliport_navaid, DesignatedPoint,
AirportHeliport_InformationService, AirportHeliport_AirportGroundService, NavaidEquipment, Azimuth, DME, DirectionFinder,
...
RunwayDirectionLightSystemTimeSlice, AirTrafficControlServiceTimeSlice, GroundTrafficControlServiceTimeSlice, procedure,
proceduretimeslice, runwayelement, channel_frequency_pair, GroundBasedAugmentationSystem, airspace_note,
GridMORA, HoldingPatternDistance, HoldingPatternDuration, HoldingPatternLength, HoldingPattern, Note, CallSignTranslate,
Airspace_SearchRescueService, Airspace_AirTrafficControlService, AirportHeliport_GroundTrafficControlService,
AirportHeliport_AirTrafficControlService, RouteAvailability, Airspace_InformationService, RouteSegment_Note,
HoldingPatternTimeSlice, RadioCommunicationOperationalStatus, TerminalSegmentPoint,
"ARP_data", "ALS_data", "NAV_data", "TPM_data", "TPT_data", "TRA_data", "RWY_data", "MVL_data", "RSA_data", "CTA_data",
"CAA_data", "CTB_data", "CTC_data", "RDA_data", "RDC_data", "CTR_data", "TMA_data", "UAA_data", "FIR_data", "SECTOR_data",
"MET_data", "HOLD_data", "1degree_data", "2degree_data", Translate, RunwayCentrelinePoint, RunwayCentrelinePointTimeSlice,
PointReference, AngleIndication, DistanceIndication, ProcedureTransition, SegmentLeg, InstrumentApproachProcedure, AngleUse,
instrumentapproachproceduretimeslice, ProcedureTransitionLeg, MissedApproachLeg, MissedApproachLegTimeSlice, DepartureLeg,
DepartureLegTimeSlice, StandardInstrumentDeparture, StandardInstrumentDepartureTimeSlice, PointReferenceDistanceIndication,
SegmentLegTimeSlice, SegmentPointReference, StandardInstrumentArrival, StandardInstrumentArrivalTimeSlice, FinalLeg,
FinalLegTimeSlice, InitialLeg, InitialLegTimeSlice, ArrivalLeg, ArrivalLegTimeSlice, ArrivalLegTimeSlice, ApproachLeg,
ApproachLegTimeSlice, AircraftCharacteristic, SpecialNavigationSystem, SpecialNavigationStation,
AirportHeliport, Navaid, NavaidEquipment, HoldingPattern
CASCADE;
...
CodeDirectionReferenceBaseType, CodeSpeedReferenceBaseType, CodeAircraftCategoryBaseType, CodeSpecialNavigationSystemBaseTypeBaseType
CASCADE;
...
CodeSpecialNavigationSystemType, IncludingValueType
CASCADE;

DROP FUNCTION if EXISTS dd2dms(p_ddecdeg double precision, laltlon varchar), set_date_filter CASCADE;
...
CREATE DOMAIN CodeCardinalDirectionBaseType as varchar(10) CHECK (VALUE ~ '(N|NE|E|SE|S|SW|W|NW|NNE|ENE|ESE|SSE|SSW|WSW|WNW|NNW|OTHER)'::text);
...
CREATE DOMAIN CodeRunwayPointRoleBaseType as varchar (20) CHECK (VALUE ~ '(START|THR|DISTHR|TDZ|MID|END|START_RUN|LAHSO|
ABEAM_GLIDESLOPE|ABEAM_PAR|ABEAM_ELEVATION|ABEAM_TDR|ABEAM_RER|OTHER)'::text);
...
CREATE DOMAIN CodeProcedurePhaseBaseType as varchar (20) CHECK (VALUE ~ '(RWY|COMMON|EN_ROUTE|APPROACH|FINAL|MISSED|
MISSED_P|MISSED_S|ENGINE_OUT|OTHER)'::text);
...
CREATE DOMAIN CodeApproachBaseType as varchar (20) CHECK (VALUE ~ '(ASR|ARA|ARSR|PAR|ILS|ILS_DME|ILS_PRM|LDA|LDA_DME|
LOC|LOC_BC|LOC_DME|LOC_DME_BC|MLS|MLS_DME|NDB|NDB_DME|SDF|TLS|VOR|VOR_DME|TACAN|VORTAC|DME|DME_DME|RNP|GPS|GLONASS|GALILEO|RNAVIGS
|OTHER)'::text);
...
CREATE DOMAIN CodeApproachEquipmentAdditionalBaseType as varchar (20) CHECK (VALUE ~ '(ADF|DME|RADAR|RADARDME|VORLOC|
DUALVORDME|DUALADF|ADFMA|SPECIAL|DUALVHF|GPSRNP3|ADFILS|DUALADF_DME|RADAR_RNAV|OTHER)'::text);
create TYPE CodeApproachEquipmentAdditionalType AS (
    nilReason NilReasonEnumeration,
    value CodeApproachEquipmentAdditionalBaseType
    );

CREATE DOMAIN CodeMissedApproachBaseType as varchar (20) CHECK (VALUE ~ '(PRIMARY|SECONDARY|ALTERNATE|TACAN|TACANALT|ENGINEOUT
|OTHER)'::text);
...
CREATE DOMAIN CodeSegmentPathBaseType as varchar (20) CHECK (VALUE ~ '(AF|HF|HA|HM|IF|PI|PT|TF|CA|CD|CI|CR|CF|DF|FA|FC|FT|FM|VM|FD|VR|VD|VI|VA|RF|OTHER)'::text);;
...
DROP SEQUENCE IF EXISTS auto_id_timesheet, auto_id_city, auto_id_point, auto_id_significant_point, auto_id_curve,
...
auto_id_airspace_volume, auto_id_telephone_contact, auto_ground_lighting_availability, auto_idPropertiesWithSchedule,
auto_id_timeslice, auto_id_HoldingPatternLength, auto_id_HoldingPatternDistance, auto_id_HoldingPatternDuration,
    auto_id_translate, auto_id_callsignTranslate CASCADE;

CREATE SEQUENCE auto_idPropertiesWithSchedule;
...
(
    ...
    timeslice_uuid                  id DEFAULT uuid_generate_v4(),
    _transasID                      VARCHAR(20),
    validTimeBegin                  DateType,
    validTimeEnd                    DateType,
    latitude                        latitude,
...
(
  id INTEGER PRIMARY KEY DEFAULT nextval('auto_idPropertiesWithSchedule')
);
...
CREATE TABLE RunwayElement
(
  uuidRunway            id REFERENCES Runway (uuid),
  type                  CodeRunwayElementType,
  length            	ValDistanceType,
...
CREATE TABLE Airspace_InformationService
(
    uuidairspc             id REFERENCES Airspace (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidInformationService id REFERENCES InformationService (uuid)
);
...
CREATE TABLE Airspace_AirTrafficControlService
(
    uuidairspc                      id REFERENCES Airspace (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidAirTrafficControlService    id REFERENCES AirTrafficControlService (uuid)
);


CREATE TABLE Airspace_SearchRescueService
(
    uuidairspc                  id REFERENCES Airspace (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidSearchRescueService     id REFERENCES SearchRescueService (uuid)
);


CREATE TABLE AirportHeliport_GroundTrafficControlService
(
    uuidarphlp                      id REFERENCES AirportHeliport (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidGroundTrafficControlService id REFERENCES GroundTrafficControlService (uuid)
);


CREATE TABLE AirportHeliport_AirTrafficControlService
(
    uuidarphlp                      id REFERENCES AirportHeliport (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidAirTrafficControlService    id REFERENCES AirTrafficControlService (uuid)
);
...
(
  idTimeSlice       INTEGER NOT NULL PRIMARY KEY REFERENCES TimeSlice (id) ON DELETE CASCADE ON UPDATE CASCADE,
  uuid              id REFERENCES Navaid (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
  type              CodeNavaidServiceType,
  designator        CodeNavaidDesignatorType,
  name              TextNameType,
  flightChecked     CodeYesNoType,
...
(
    idTimeSlice                 INTEGER NOT NULL REFERENCES TimeSlice (id),
...
(
  ...
  idPoint           INTEGER, --- REFERENCES Point (id) ON DELETE CASCADE ON UPDATE CASCADE,
  uuidrwyclpt       id REFERENCES RunwayCentrelinePoint (uuid),
...
(
    id                INTEGER PRIMARY KEY REFERENCES PropertiesWithSchedule (id) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidrtsegment     id REFERENCES RouteSegment (uuid),
...
CREATE TABLE HoldingPatternDistance
(
    id    INTEGER PRIMARY KEY DEFAULT nextval('auto_id_HoldingPatternDistance'),
    length ValDistanceType
);

CREATE TABLE HoldingPatternDuration
(
    id    INTEGER PRIMARY KEY DEFAULT nextval('auto_id_HoldingPatternDuration'),
    duration ValDurationType
);

CREATE TABLE HoldingPatternLength
(
    id                          INTEGER PRIMARY KEY  DEFAULT nextval('auto_id_HoldingPatternLength'),
    idSegmentPoint              INTEGER REFERENCES EnRouteSegmentPoint (id) ON DELETE CASCADE ON UPDATE CASCADE,
    idHoldingPatternDistance    INTEGER REFERENCES HoldingPatternDistance (id) ON DELETE CASCADE ON UPDATE CASCADE,
    idHoldingPatternDuration    INTEGER REFERENCES HoldingPatternDuration (id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
(
    ...
    idSegmentPoint         INTEGER REFERENCES EnRouteSegmentPoint (id) ON DELETE CASCADE ON UPDATE CASCADE,
    idCurve                INTEGER REFERENCES Curve (id) ON DELETE CASCADE ON UPDATE CASCADE,
    idHoldingPatternLength INTEGER REFERENCES HoldingPatternLength (id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
(
  idTimeSlice INTEGER NOT NULL PRIMARY KEY REFERENCES NavaidEquipment (idTimeSlice) ON DELETE CASCADE ON UPDATE CASCADE,
  uuid                      id REFERENCES NavaidEquipment (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
...
(
  idTimeSlice          INTEGER NOT NULL PRIMARY KEY REFERENCES TimeSlice (id),
  uuid                 id REFERENCES VerticalStructure (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
  name                 TextNameType,
...
(
  idTimeSlice           INTEGER NOT NULL PRIMARY KEY REFERENCES NavaidEquipment (idTimeSlice),
  uuid                  id REFERENCES NavaidEquipment (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
...
(
    id          INTEGER NOT NULL PRIMARY KEY DEFAULT nextval('auto_id_translate'),
    idTimeSlice INTEGER NOT NULL REFERENCES TimeSlice (id),
    translation VARCHAR
);


CREATE TABLE CallSignTranslate
(
    id          INTEGER NOT NULL PRIMARY KEY DEFAULT nextval('auto_id_callsignTranslate'),
    idObject    INTEGER REFERENCES CallsignDetail (id),
    translation VARCHAR
);

CREATE TABLE TerminalSegmentPoint
(
    id                      INTEGER PRIMARY KEY REFERENCES SegmentPoint (id) ON DELETE CASCADE ON UPDATE CASCADE,
...
(
    idTimeSlice                 INTEGER NOT NULL REFERENCES TimeSlice (id),
    uuid                        id REFERENCES InstrumentApproachProcedure (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
...
(
    idTimeSlice             INTEGER NOT NULL PRIMARY KEY REFERENCES TimeSlice (id),
    uuid                    id REFERENCES SegmentLeg (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE ApproachLegTimeSlice
(
    idTimeSlice             INTEGER NOT NULL PRIMARY KEY REFERENCES TimeSlice (id),
    uuid                    id REFERENCES ApproachLeg (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
...
create table MissedApproachLegTimeSlice
(
    idTimeSlice             INTEGER NOT NULL REFERENCES TimeSlice (id),
    uuid                    id REFERENCES MissedApproachLeg (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
...
create table DepartureLegTimeSlice
(
    idTimeSlice             INTEGER NOT NULL REFERENCES TimeSlice (id),
    uuid                    id REFERENCES DepartureLeg (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidDeparture           id
);
...
(
    idTimeSlice             INTEGER NOT NULL REFERENCES TimeSlice (id),
    uuid                    id REFERENCES StandardInstrumentDeparture (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE ArrivalLegTimeSlice
(
    idTimeSlice     INTEGER NOT NULL REFERENCES TimeSlice (id),
    uuid            id REFERENCES ArrivalLeg (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidArrival     id
);
...
CREATE TABLE InitialLegTimeSlice
(
    idTimeSlice     INTEGER NOT NULL REFERENCES TimeSlice (id),
    uuid            id REFERENCES InitialLeg (uuid) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE TABLE FinalLeg
(
    uuid    id PRIMARY KEY DEFAULT uuid_generate_v4()
);
CREATE TABLE FinalLegTimeSlice
(
    idTimeSlice     INTEGER NOT NULL REFERENCES TimeSlice (id),
    uuid            id REFERENCES FinalLeg (uuid) ON DELETE CASCADE ON UPDATE CASCADE
);
...
(
    idTimeSlice     INTEGER NOT NULL REFERENCES TimeSlice (id),
    uuid            id REFERENCES StandardInstrumentArrival (uuid) ON DELETE CASCADE ON UPDATE CASCADE,
    designator      TextSIDSTARDesignatorType
);
...
(
    uuid                    id,
    idElevatedPoint         INTEGER REFERENCES ElevatedPoint (id) ON DELETE CASCADE ON UPDATE CASCADE,
    uuidSystemChain         id REFERENCES SpecialNavigationSystem (uuid) ON DELETE CASCADE ON UPDATE CASCADE
);
